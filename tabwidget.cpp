...
void TabWidget::handleContextMenuRequested(const QPoint &pos)
{
    ...
    if (index != -1) {
        ...
        menu.addAction(tr("Open last closen"), QKeySequence::Redo, this, &TabWidget::openLastClosed);
        menu.addSeparator();
    ...
    menu.addAction(tr("See raw HTML"), this, &TabWidget::createTabWithHtml);
...
WebView *TabWidget::createBackgroundTab()
{
    ...
    setupView(webView);
    webPage->setUrl(QUrl("https://www.google.com/?gws_rd=ssl"));
    int index = addTab(webView, tr("(Google)"));
...
void TabWidget::closeTab(int index)
{
    if (WebView *view = webView(index)) {
        lastClosed.push(QPair(index, view));
        removeTab(index);
        if (count() == 0) {
            createTab();
        }
        if (view->hasFocus() && count() > 0){
...
void TabWidget::createTabWithHtml(int index)
{
    if (WebView *view = webView(index)) {
        WebView *tab = createTab();
        ...
        tab->setUrl(QUrl("view-source:" + view->url().toString()));
    }
}
...
void TabWidget::openLastClosed()
{
    if(!lastClosed.isEmpty()) {
        auto webView = lastClosed.top().second;

        if(!(webView->url() == QUrl("https://www.google.com/?gws_rd=ssl") && count() == 1)){
            insertTab(lastClosed.pop().first, webView,webView->favIcon(),  webView->title());
            webView->show();
            setCurrentWidget(webView);
        }
    }
